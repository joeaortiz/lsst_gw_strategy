...
def parse_dump(infile):
    with open(infile, 'r') as inp:
        # Prepare data type for struc. array
        header = []
        while 'VARNAMES:' not in header:
            header = inp.readline().split()
        params = header
        cols = [i for i in range(len(params)) if params[i] in dtypes_dump.keys()]
        dtype = [(params[col], dtypes_dump[params[col]]) for col in cols]

        # Filter out anything that isn't an observation
        filtered_inp = ifilter(lambda line: line.startswith('SN:'), inp)
        obs = np.genfromtxt(filtered_inp, usecols=cols, dtype=dtype)
